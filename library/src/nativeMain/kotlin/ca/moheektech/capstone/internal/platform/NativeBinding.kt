...
internal class NativeCapstoneBinding(
    ...
    override fun setOption(option: CapstoneOption): CapstoneResult<Unit> = runCatching {
        val err = cs_option(
            handle,
            option.optType.convert(),
            option.optValue.convert()
        )
    ...
    private fun convertInstruction(insn: cs_insn): Instruction {
        ...
        val detail = if (detailEnabled && insn.detail != null) {
    ...
    private fun convertArmDetail(arm: cs_arm): ArmInstructionDetail {
        val operands = buildList {
            for (i in 0 until arm.op_count.toInt()) {
                ...
                when (type) {
                    ArmOpType.REG, ArmOpType.SYSREG, ArmOpType.BANKEDREG -> {
                        val regId = op.reg
                        val regName = cs_reg_name(handle, regId.convert())?.toKString()
                        reg = Register(regId, regName)
                    }
                    ArmOpType.IMM, ArmOpType.CIMM, ArmOpType.PIMM -> {
                        imm = op.imm.toInt()
                    }
                    ArmOpType.FP -> {
                        fp = op.fp
                    ...
                    ArmOpType.MEM -> {
                        ...
                        val baseReg = if (baseRegId != 0u) {
                             Register(baseRegId.toInt(), cs_reg_name(handle, baseRegId.convert())?.toKString())
                        } else null
                        
                        val indexRegId = m.index
                        val indexReg = if (indexRegId != 0u) {
                            Register(indexRegId.toInt(), cs_reg_name(handle, indexRegId.convert())?.toKString())
                        } else null
    ...
    private fun convertAArch64Detail(aarch64: cs_aarch64): AArch64InstructionDetail {
        val operands = buildList {
            for (i in 0 until aarch64.op_count.toInt()) {
                ...
                val type = AArch64OpType.entries.firstOrNull { it.value.toInt() == typeRaw } ?: AArch64OpType.INVALID
                ...
                when (type) {
                    AArch64OpType.REG, AArch64OpType.REG_MRS, AArch64OpType.REG_MSR -> {
                        val regId = op.reg
                        val regName = cs_reg_name(handle, regId.convert())?.toKString()
                        reg = Register(regId.toInt(), regName)
                    }
                    AArch64OpType.IMM, AArch64OpType.CIMM -> {
                        imm = op.imm
                    }
                    AArch64OpType.FP -> {
                        fp = op.fp
                ...
                add(AArch64Operand(
                    ...
                    access = AccessType.fromValue(op.access.toInt()),
                    vectorIndex = op.vector_index,
                    vas = AArch64VectorLayout.entries.firstOrNull { it.value.toInt() == op.vas.toInt() } ?: AArch64VectorLayout.INVALID,
                    shifter = AArch64Shifter.entries.firstOrNull { it.value.toInt() == op.shift.type.toInt() } ?: AArch64Shifter.INVALID,
                    shiftValue = op.shift.value.toInt(),
                    extender = AArch64Extender.entries.firstOrNull { it.value.toInt() == op.ext.toInt() } ?: AArch64Extender.INVALID,
                    reg = reg,
                    imm = imm,
                    fp = fp,
                    mem = mem,
        ...
        return AArch64InstructionDetail(
            cc = AArch64ConditionCode.entries.firstOrNull { it.value.toInt() == aarch64.cc.convert<Int>() } ?: AArch64ConditionCode.Invalid,
    ...
    private fun convertX86Detail(x86: cs_x86): X86InstructionDetail {
            ...
            for (i in 0 until x86.op_count.toInt()) {
                ...
                when (type) {
                    ...
                    X86OpType.MEM -> {
                        ...
                        val segmentReg = if (segmentRegId != 0u) {
                            Register(segmentRegId.toInt(), cs_reg_name(handle, segmentRegId.convert())?.toKString())
                        } else null
        ...
        return X86InstructionDetail(
            ...
            sib = x86.sib.toByte(),
            disp = x86.disp,
            sibIndex = if (x86.sib_index != 0u) Register(x86.sib_index.toInt(), cs_reg_name(handle, x86.sib_index.convert())?.toKString()) else null,
            sibScale = x86.sib_scale.toInt(),
            sibBase = if (x86.sib_base != 0u) Register(x86.sib_base.toInt(), cs_reg_name(handle, x86.sib_base.convert())?.toKString()) else null,
            operands = operands,
