...
internal class JnaCapstoneBinding(
    ...
    init {
        val err = CapstoneLibrary.INSTANCE.cs_open(
            architecture.value,
            mode.value,
            handleRef
        )
    ...
    override fun disasm(
        ...
        val disasmCount = CapstoneLibrary.INSTANCE.cs_disasm(
            handle,
            code,
    ...
    override fun disasmIter(
        ...
        val insn = CapstoneLibrary.INSTANCE.cs_malloc(handle)
        ...
        try {
            ...
            val result = CapstoneLibrary.INSTANCE.cs_disasm_iter(
                handle,
                codePtrRef,
                sizeRef,
                addrRef,
            ...
            if (result) {
                ...
                // Update position - cs_disasm_iter already updated the refs, but we need to sync our position
    ...
    override fun setOption(option: CapstoneOption): CapstoneResult<Unit> = runCatching {
        val err = CapstoneLibrary.INSTANCE.cs_option(
            handle,
            option.optType,
            NativeLong(option.optValue.toLong())
        )
    ...
    private fun convertAArch64Detail(aarch64: Arm64UnionOpInfo): AArch64InstructionDetail {
        val operands = buildList {
            for (i in 0 until aarch64.op_count.toInt()) {
                ...
                val type = AArch64OpType.entries.firstOrNull { it.toInt() == op.type }
                ...
                add(AArch64Operand(
                    ...
                    access = AccessType.fromValue(op.access.toInt()),
                    vectorIndex = op.vector_index,
                    vas = AArch64VectorLayout.entries.firstOrNull { it.toInt() == op.vas }
                        ?: AArch64VectorLayout.INVALID,
                    shifter = AArch64Shifter.entries.firstOrNull { it.toInt() == op.shift.type }
                        ?: AArch64Shifter.INVALID,
                    shiftValue = op.shift.value,
                    extender = AArch64Extender.entries.firstOrNull { it.toInt() == op.ext }
        ...
        return AArch64InstructionDetail(
            cc = AArch64ConditionCode.entries.firstOrNull { it.toInt() == aarch64.cc }
    ...
    private fun convertX86Detail(x86: X86UnionOpInfo): X86InstructionDetail {
        ...
        return X86InstructionDetail(
            ...
            sib = x86.sib,
            disp = x86.disp,
            sibIndex = if (x86.sib_index != 0) Register(x86.sib_index, CapstoneLibrary.INSTANCE.cs_reg_name(handle, x86.sib_index)) else null,
            sibScale = x86.sib_scale.toInt(),
            sibBase = if (x86.sib_base != 0) Register(x86.sib_base, CapstoneLibrary.INSTANCE.cs_reg_name(handle, x86.sib_base)) else null,
            operands = operands,
