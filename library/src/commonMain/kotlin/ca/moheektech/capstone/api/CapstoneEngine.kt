 ...
 *
 * This is the primary interface for disassembling machine code.
 * Use the [build] factory method to create instances with configuration.
 *
 * The engine must be closed after use to release resources. Use the [use] extension
 * function for automatic resource management:
 *
 * ```kotlin
 ...
 */
class CapstoneEngine internal constructor(
    ...
    fun disassembleOne(
        ...
        checkNotClosed()
        return binding.disasmIter(code, position).map { insn ->
            insn?.let { upgradeInstruction(it) }
        }
    }
     ...
     *
     * After closing, the engine cannot be used anymore.
     * Prefer using the [use] extension function for automatic cleanup.
     */
    override fun close() {
        if (!closed) {
            binding.close()
    ...
    private fun upgradeInstruction(insn: Instruction): Instruction {
        ...
        return try {
            when (architecture) {
                Architecture.ARM -> ArmInstruction(
                    id = insn.id,
                ...
                Architecture.ARM64 -> Arm64Instruction(
                    id = insn.id,
                    address = insn.address,
                    size = insn.size,
                    bytes = insn.bytes,
                ...
                Architecture.X86 -> X86Instruction(
                    id = insn.id,
                    address = insn.address,
                    size = insn.size,
                    bytes = insn.bytes,
    ...
    companion object {
        ...
        fun build(
            ...
            return CapstoneBuilder(architecture, mode)
                .apply(configure)
                .build()
        }
