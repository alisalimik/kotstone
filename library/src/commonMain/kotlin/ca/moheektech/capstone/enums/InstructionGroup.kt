 ...
 *
 * Groups classify instructions by their behavior (jump, call, return, etc.)
 * Maps to cs_group_type enum from capstone.h
 */
enum class InstructionGroup(val value: Int) {
    ...
    companion object {
         ...
         */
        fun fromValue(value: Int): InstructionGroup? =
            entries.firstOrNull { it.value == value }

        /**
         * Convert list of integer group IDs to InstructionGroup list
         */
        fun fromValues(values: List<Int>): List<InstructionGroup> =
            values.mapNotNull { fromValue(it) }
    }
}
