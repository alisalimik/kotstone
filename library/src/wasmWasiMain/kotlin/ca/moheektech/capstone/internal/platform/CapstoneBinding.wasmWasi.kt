...
/**
 * WASI-compatible Capstone binding implementation.
 * Uses direct pointer memory access via UnsafeWasmMemory and CapstoneWasi raw functions.
 */
internal class WasmWasiCapstoneBinding(
    ...
    override fun disasm(
        ...
        try {
            ...
            if (disasmCount <= 0) {
                ...
                throw RuntimeException("DEBUG: cs_disasm returned 0. Errno: $errno. CodeSize: ${code.size}. Handle: $handle")
            }
            ...
            if (disasmCount > 0) {
                ...
                for (i in 0 until disasmCount) {
                    ...
                    instructions.add(InternalInstruction(
                        id = id,
                        address = address,
                        size = size,
                        bytes = bytes,
    ...
    override fun disasmIter(
        ...
        // This avoids managing pointer-to-pointers for cs_disasm_iter which is tricky in Wasm without a helper
