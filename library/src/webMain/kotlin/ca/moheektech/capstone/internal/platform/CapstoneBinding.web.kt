...
// internal expect fun Long.toBigInt(): JsBigInt // Not needed if we use helper or split 32-bit reading
...
internal actual fun getPlatformVersion(): Pair<Int, Int> {
    ...
    try {
        ...
        val major = (module.getValue(majorPtr, "i32") as JsAny).toInt()
        val minor = (module.getValue(minorPtr, "i32") as JsAny).toInt()
...
internal fun getModuleInstance(): CapstoneModuleInstance {
    return moduleInstance ?: error("Capstone module not initialized. Call initializeCapstoneModule() first.")
}
...
internal class WasmCapstoneBinding(
    ...
    private val handlePtr: Int
    private val handle: Int
    private var detailEnabled = false

    init {
        handlePtr = module._malloc(4)  // Allocate space for size_t handle
        ...
        if (err != ErrorCode.OK.value) {
            ...
            throw CapstoneError.UnsupportedArchitecture(
                architecture,
                "Failed to open Capstone: $errorMsg"
            )
        }

        handle = (module.getValue(handlePtr, "i32") as JsAny).toInt()
    }
    ...
    override fun disasm(
        ...
        try {
            ...
            // Call cs_disasm
            val disasmCount = module._cs_disasm(
                handle,
                codePtr,
                code.size,
            ...
            if (disasmCount == 0) {
                ...
                try {
                    ...
                    val insnPtr = (module.getValue(insnPtrPtr, "i32") as JsAny).toInt()
        ...
        try {
            ...
            try {
                ...
                try {
                    ...
                    val result = module._cs_disasm_iter(
                        handle,
                        codePtrPtr,
                        sizePtr,
                        addrPtr,
    ...
    private fun readCString(ptr: Int, maxLen: Int): String {
        ...
        for (i in 0 until maxLen) {
            val byte = (module.getValue(ptr + i, "i8") as JsAny).toInt().toByte()
    ...
    private fun readInt(ptr: Int): Int {
        return (module.getValue(ptr, "i32") as JsAny).toInt()
    }
    ...
    private fun readShort(ptr: Int): Int {
        return (module.getValue(ptr, "i16") as JsAny).toInt()
    }
    ...
    private fun convertDetail(detailPtr: Int): InstructionDetail {
        ...
        val regsReadCount = (module.getValue(detailPtr + 40, "i8") as JsAny).toInt() and 0xFF
        ...
        val regsWriteCount = (module.getValue(detailPtr + 81, "i8") as JsAny).toInt() and 0xFF
        ...
        val groupsCount = (module.getValue(detailPtr + 90, "i8") as JsAny).toInt() and 0xFF
        val groups = buildList {
            for (i in 0 until groupsCount) {
                val groupId = (module.getValue(detailPtr + 82 + i, "i8") as JsAny).toInt() and 0xFF
        ...
        return InstructionDetail(
            ...
            groups = groups,
            writeback = (module.getValue(detailPtr + 91, "i8") as JsAny).toInt() != 0,
    ...
    private fun convertAArch64Detail(ptr: Int): AArch64InstructionDetail {
        // cs_arm64 structure
        val ccVal = (module.getValue(ptr, "i8") as JsAny).toInt()
        val updateFlags = (module.getValue(ptr + 1, "i8") as JsAny).toInt() != 0
        val writeback = (module.getValue(ptr + 2, "i8") as JsAny).toInt() != 0
        val opCount = (module.getValue(ptr + 3, "i8") as JsAny).toInt() and 0xFF
    ...
    private fun convertAArch64Operand(ptr: Int): AArch64Operand {
        ...
        val data = when (opType) {
            AArch64OpType.REG -> readInt(ptr + 32)
            AArch64OpType.IMM, AArch64OpType.CIMM -> readLong(ptr + 32)
            AArch64OpType.FP -> {
                // Read as double
        ...
        val reg = if(opType == AArch64OpType.REG) Register(data.toInt(), regName(data.toInt())) else null
        val imm = if(opType == AArch64OpType.IMM || opType == AArch64OpType.CIMM) data as Long else null
    ...
    private fun convertArmDetail(ptr: Int): ArmInstructionDetail {
        val usermode = (module.getValue(ptr, "i8") as JsAny).toInt() != 0
        val vectorSize = readInt(ptr + 4)
        val vectorDataVal = readInt(ptr + 8)
        val cpsModeVal = (module.getValue(ptr + 12, "i8") as JsAny).toInt()
        val cpsFlagVal = (module.getValue(ptr + 13, "i8") as JsAny).toInt()
        val ccVal = (module.getValue(ptr + 14, "i8") as JsAny).toInt()
        val updateFlags = (module.getValue(ptr + 15, "i8") as JsAny).toInt() != 0
        val writeback = (module.getValue(ptr + 16, "i8") as JsAny).toInt() != 0
        val memBarrierVal = (module.getValue(ptr + 17, "i8") as JsAny).toInt()
        val opCount = (module.getValue(ptr + 18, "i8") as JsAny).toInt() and 0xFF
    ...
    private fun convertArmOperand(ptr: Int): ArmOperand {
        ...
        val dataVal = when (opType) {
            ArmOpType.REG, ArmOpType.SYSREG -> readInt(ptr + 16)
            ArmOpType.IMM, ArmOpType.CIMM, ArmOpType.PIMM -> readInt(ptr + 16)
            ArmOpType.FP -> {
        ...
        val reg = if(opType == ArmOpType.REG || opType == ArmOpType.SYSREG) Register(dataVal.toInt(), regName(dataVal.toInt())) else null
        val imm = if(opType == ArmOpType.IMM || opType == ArmOpType.CIMM || opType == ArmOpType.PIMM) dataVal as Int else null
        ...
        val setend = if(opType == ArmOpType.SETEND) ArmSetEndType.entries.firstOrNull { it.toInt() == dataVal as Int } ?: ArmSetEndType.INVALID else ArmSetEndType.INVALID

        val subtracted = (module.getValue(ptr + 36, "i8") as JsAny).toInt() != 0
        val access = readInt(ptr + 40)
        val neonLane = (module.getValue(ptr + 44, "i8") as JsAny).toInt().toByte()
    ...
    private fun convertX86Detail(ptr: Int): X86InstructionDetail {
        ...
        val prefix = buildList {
            for (i in 0 until 4) {
                val pVal = (module.getValue(ptr + i, "i8") as JsAny).toInt() and 0xFF
                if (pVal != 0) {
        ...
        val opcode = ByteArray(4) { i -> (module.getValue(ptr + 4 + i, "i8") as JsAny).toInt().toByte() }

        val rex = (module.getValue(ptr + 8, "i8") as JsAny).toInt().toByte()
        val addrSize = (module.getValue(ptr + 9, "i8") as JsAny).toInt()
        val modRm = (module.getValue(ptr + 10, "i8") as JsAny).toInt().toByte()
        val sib = (module.getValue(ptr + 11, "i8") as JsAny).toInt().toByte()

        val disp = readLong(ptr + 12)

        val sibIndexId = readInt(ptr + 20)
        val sibScale = (module.getValue(ptr + 24, "i8") as JsAny).toInt()
        val sibBaseId = readInt(ptr + 28)

        val xopCcVal = readInt(ptr + 32)
        val sseCCVal = readInt(ptr + 36)
        val avxCcVal = readInt(ptr + 40)
        val avxSae = (module.getValue(ptr + 44, "i8") as JsAny).toInt() != 0
        ...
        val opCount = (module.getValue(ptr + 60, "i8") as JsAny).toInt() and 0xFF
    ...
    private fun convertX86Operand(ptr: Int): X86Operand {
        ...
        val reg = if(opType == X86OpType.REG) Register(dataVal.toInt(), regName(dataVal.toInt())) else null
        val imm = if(opType == X86OpType.IMM) dataVal as Long else null
        val mem = if(opType == X86OpType.MEM) convertX86MemOp(ptr + 4) else null

        val size = (module.getValue(ptr + 28, "i8") as JsAny).toInt()
        val access = readInt(ptr + 32)
        val avxBcastVal = readInt(ptr + 36)
        val avxZeroOpmask = (module.getValue(ptr + 40, "i8") as JsAny).toInt() != 0
