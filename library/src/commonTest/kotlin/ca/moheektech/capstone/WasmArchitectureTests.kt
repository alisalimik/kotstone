...
class WasmArchitectureTests {
        ...
        CapstoneEngine.build(Architecture.WASM, Mode.LITTLE_ENDIAN).use { cs ->
            ...
            if (result.isSuccess) {
                ...
                if (insns.isNotEmpty()) {
                    ...
                    assertTrue(mnemonic == "local.get" || mnemonic == "get_local", "Expected local.get or get_local, got $mnemonic")
                }
            }
        }
    }
    ...
    fun testWasmLocalSet() = runTest {
        ...
        CapstoneEngine.build(Architecture.WASM, Mode.LITTLE_ENDIAN).use { cs ->
            ...
            if (result.isSuccess) {
                ...
                if (insns.isNotEmpty()) {
                    val mnemonic = insns[0].mnemonic
                    assertTrue(mnemonic == "local.set" || mnemonic == "set_local", "Expected local.set or set_local, got $mnemonic")
                }
            }
        }
    }
    ...
    fun testWasmCall() = runTest {
        ...
        CapstoneEngine.build(Architecture.WASM, Mode.LITTLE_ENDIAN) {
            detail = true
        }.use { cs ->
            val result = cs.disassemble(code, address = 0x0)
